...
def plot_data(hit_data, logy_toggle, logz_toggle, color_palette, heatmap_color,clear_button, h_totals_hits):
	...
	if callback_context.triggered_id == "clear_button":
		empty_fig = make_subplots(
			...
			subplot_titles=[
				"last position wires", "last position grids", "last wires adc", "last grids adc",
				"total position wires", "total position grids", "total wires adc", "total grids adc"
			]
		)
		...
		return empty_fig, {"h1": [0]*channels_x, "h2": [0]*channels_y, "h3": np.zeros((channels_x, 128)).tolist(), "h4": np.zeros((channels_y, 128)).tolist()}
	
	
	updated_at = shared_data.get("hit_updated_at", 0.0)
	if not hit_data or updated_at == last_seen_timestamp:
		return dash.no_update, h_totals_hits
	print("hits " + str(updated_at) + "         " + str(updated_at-last_seen_timestamp))
	last_seen_timestamp = updated_at


	df_hits = pd.DataFrame(hit_data)
	hits0 = df_hits.query("plane == 0 and (det >= 0 and det <=1)")
	hits1 = df_hits.query("plane == 1 and (det >= 0 and det <=1)")
	
	fig = make_subplots(rows = 2, cols = 4, horizontal_spacing=0.06, vertical_spacing=0.1, subplot_titles = ("last position wires", "last position grids", "last wires adc", "last grids adc", "total position wires", "total position grids", "total wires adc", "total grids adc"))
	...
	h1 = np.histogram(hits0['pos']+hits0['det']*120, bins = channels_x, range = [0.0, channels_x])
	h1_total = np.array(h_totals_hits.get("h1", [0]*channels_x))
	h1_total += h1[0]
	
	h2 = np.histogram(hits1['pos']+hits1['det']*12, bins = channels_y, range = [0.0, channels_y])
	h2_total = np.array(h_totals_hits.get("h2", [0]*channels_y))
	h2_total += h2[0]

	h3 = np.histogram2d(hits0['pos']+hits0['det']*120, hits0['adc'], bins = [channels_x, 128], range = np.array([(0, channels_x), (0,1024)]))
	h3_total = np.array(
	...
	h4 = np.histogram2d(hits1['pos']+hits1['det']*12, hits1['adc'], bins = [channels_y, 128], range = np.array([(0, channels_y), (0,1024)]))
	h4_total = np.array(
	...
	fig.update_xaxes(title_text=("wires [pitch x=22 mm, z=10 mm]"), row = 1, col = 3)
	fig.update_yaxes(title_text=("adc"), row = 1, col = 3)
	
	fig.update_xaxes(title_text=("grids [pitch y=25 mm]"), row = 1, col = 4)
	...
	fig.update_xaxes(title_text=("wires [pitch x=22 mm, z=10 mm]"), row = 2, col = 3)
	fig.update_yaxes(title_text=("adc"), row = 2, col = 3)
	
	fig.update_xaxes(title_text=("grids [pitch y=25 mm]"), row = 2, col = 4)
