...
int main(int argc, char **argv) {
  ...
  if (m_config.ParseCommandLine(argc, argv)) {
    ...
    if (!m_config.pIsPcap || m_config.pDataFormat != "ESS") {
      return 0;
    }
  } else {
    return -1;
  ...
  while (doContinue &&
    ...
    if (temp_pulseTime - pulseTime > 1.0E+06) {
      m_stats.IncrementCounter("NumberOfTriggers", 384);
      ...
      if (m_config.pUseBunchFile == true) {
        ...
        if (itStart != itEnd) {
          ...
          if (cnt == 1) {
            m_stats.IncrementCounter("NumberOfMatchedTriggers", 384);
            if (pulseIntensity >= 1.0E+7 && pulseIntensity < 1.0E+8) {
              m_stats.IncrementCounter("NumberOfBunches_1E7_1E8", 384);
              m_stats.IncrementCounter("IntensityOfBunches_1E7_1E8", 384,
                                       pulseIntensity);
            } else if (pulseIntensity >= 1.0E+8 && pulseIntensity < 1.0E+9) {
              m_stats.IncrementCounter("NumberOfBunches_1E8_1E9", 384);
              m_stats.IncrementCounter("IntensityOfBunches_1E8_1E9", 384,
                                       pulseIntensity);
            } else if (pulseIntensity >= 1.0E+9 && pulseIntensity < 1.0E+10) {
              m_stats.IncrementCounter("NumberOfBunches_1E9_1E10", 384);
              m_stats.IncrementCounter("IntensityOfBunches_1E9_1E10", 384,
                                       pulseIntensity);
            } else if (pulseIntensity >= 1.0E+10 && pulseIntensity < 1.0E+11) {
              m_stats.IncrementCounter("NumberOfBunches_1E10_1E11", 384);
              m_stats.IncrementCounter("IntensityOfBunches_1E10_1E11", 384,
                                       pulseIntensity);
            } else if (pulseIntensity >= 1.0E+11 && pulseIntensity < 1.0E+12) {
              m_stats.IncrementCounter("NumberOfBunches_1E11_1E12", 384);
              m_stats.IncrementCounter("IntensityOfBunches_1E11_1E12", 384,
                                       pulseIntensity);
            } else if (pulseIntensity >= 1.0E+12 && pulseIntensity < 1.0E+13) {
              m_stats.IncrementCounter("NumberOfBunches_1E12_1E13", 384);
              m_stats.IncrementCounter("IntensityOfBunches_1E12_1E13", 384,
                                       pulseIntensity);
            }
            ...
            m_stats.IncrementCounter("NumberOfDoubleMatchedTriggers", 384);
            m_config.pMapPulsetimeIntensity.emplace(
                std::make_pair(pulseTime, 0));
          }
        } else {
          m_stats.IncrementCounter("NumberOfUnMatchedTriggers", 384);
    ...
    for (int i = 0; i < hits; i++) {
      ...
      if (t0_correction == 0) {
        ...
        complete_timestamp = hit.TimeHigh * 1.0E+09 - t0_correction * 1.0E+09 +
                             hit.TimeLow * m_config.pBCTime_ns * 0.5;
      }
      ...
      double time_without_calib =
          (1.5 * m_config.pBCTime_ns - static_cast<double>(hit.TDC) *
                                           static_cast<double>(m_config.pTAC) /
                                           255.0);

      double time_with_calib = (1.5 * m_config.pBCTime_ns -
      ...
      bool result = m_Clusterer->AnalyzeHits(
          complete_timestamp, assisterId, hit.VMM, hit.Channel, hit.BC, hit.TDC,
          static_cast<uint16_t>(corrected_adc), overThreshold != 0,
  ...
  m_Clusterer->SaveDate(pcap.firstPacketSeconds, pcap.firstPacketDate,
                        ...
                        m_stats.GetCounter("NumberOfTriggers", 384));
  ...
  std::cout << "****************************************" << std::endl;
  std::cout
      << m_stats.GetCounter("NumberOfBunches_1E7_1E8", 384)
      << " bunches (between 1E+7 and 1E+8 protons), with total intensity of "
      << std::fixed << std::setprecision(12)
      << m_stats.GetCounter("IntensityOfBunches_1E7_1E8", 384) << " protons"
      << std::endl;
  std::cout
      << m_stats.GetCounter("NumberOfBunches_1E8_1E9", 384)
      << " bunches (between 1E+8 and 1E+9 protons), with total intensity of "
      << std::fixed << std::setprecision(12)
      << m_stats.GetCounter("IntensityOfBunches_1E8_1E9", 384) << " protons"
      << std::endl;
  std::cout
      << m_stats.GetCounter("NumberOfBunches_1E9_1E10", 384)
      << " bunches (between 1E+9 and 1E+10 protons), with total intensity of "
      << std::fixed << std::setprecision(12)
      << m_stats.GetCounter("IntensityOfBunches_1E9_1E10", 384) << " protons"
      << std::endl;
  std::cout
      << m_stats.GetCounter("NumberOfBunches_1E10_1E11", 384)
      << " bunches (between 1E+10 and 1E+11 protons), with total intensity of "
      << std::fixed << std::setprecision(12)
      << m_stats.GetCounter("IntensityOfBunches_1E10_1E11", 384) << " protons"
      << std::endl;
  std::cout
      << m_stats.GetCounter("NumberOfBunches_1E11_1E12", 384)
      << " bunches (between 1E+11 and 1E+12 protons), with total intensity of "
      << std::fixed << std::setprecision(12)
      << m_stats.GetCounter("IntensityOfBunches_1E11_1E12", 384) << " protons"
      << std::endl;
  std::cout
      << m_stats.GetCounter("NumberOfBunches_1E12_1E13", 384)
      << " bunches (between 1E+12 and 1E+13 protons), with total intensity of "
      << std::fixed << std::setprecision(12)
      << m_stats.GetCounter("IntensityOfBunches_1E12_1E13", 384) << " protons"
      << std::endl;
  std::cout << "****************************************" << std::endl;
